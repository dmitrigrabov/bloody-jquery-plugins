...
import {
  ...
  postAutomationsAutomationIdDuplicate,
...
import type { DuplicateAutomationResponse } from "packages/models/src/duplicateAutomationResponse.generated.ts";
...
app.post(
  ...
  async (c) => {
    ...
    const params = c.req.valid("param");
    return c.json(
      await postAutomationsAutomationIdDuplicate({ db, env: c.env, user: c.var.user, params }),
      201,
    );
  },
);
...
export type PostAutomationsAutomationIdDuplicateInput = {
  db: Db;
  env: Env;
  user: AuthUser | null;
  params: { automation_id: string };
...
export type PostAutomationsAutomationIdDuplicateHandler = (
  input: PostAutomationsAutomationIdDuplicateInput,
