...
import {
  ...
  postBroadcastsIdCancel,
  getBroadcastsIdRecipients,
  getBroadcastsIdClickedLinks,
  getSegmentsId,
  patchSegmentsId,
  deleteSegmentsId,
  getTopicsId,
...
import type { CancelBroadcastResponseSuccess } from "packages/models/src/cancelBroadcastResponseSuccess.generated.ts";
import type { ListBroadcastRecipientsResponseSuccess } from "packages/models/src/listBroadcastRecipientsResponseSuccess.generated.ts";
import type { ListBroadcastClickedLinksResponseSuccess } from "packages/models/src/listBroadcastClickedLinksResponseSuccess.generated.ts";
...
} from "packages/models/src/updateSegmentOptions.generated.ts";
import type { UpdateSegmentResponseSuccess } from "packages/models/src/updateSegmentResponseSuccess.generated.ts";
...
app.post("/broadcasts/:id/cancel", validate("param", z.object({ id: z.string() })), async (c) => {
  const db = createDb(c.env.DB);
  const params = c.req.valid("param");
  return c.json(await postBroadcastsIdCancel({ db, env: c.env, user: c.var.user, params }));
});
app.get(
  ...
  async (c) => {
    ...
    return c.json(
      await getBroadcastsIdRecipients({ db, env: c.env, user: c.var.user, params, query }),
    );
  },
);
app.get(
  ...
  async (c) => {
    ...
    const query = c.req.valid("query");
    return c.json(
      await getBroadcastsIdClickedLinks({ db, env: c.env, user: c.var.user, params, query }),
    );
  },
);
...
app.patch(
  "/segments/:id",
  validate("param", z.object({ id: z.string() })),
  validate("json", updateSegmentOptions),
  async (c) => {
    const db = createDb(c.env.DB);
    const params = c.req.valid("param");
    const body = c.req.valid("json");
    return c.json(await patchSegmentsId({ db, env: c.env, user: c.var.user, params, body }));
  },
);
...
export type PostBroadcastsIdCancelInput = {
  db: Db;
  env: Env;
  user: AuthUser | null;
  params: { id: string };
...
export type PostBroadcastsIdCancelHandler = (
  input: PostBroadcastsIdCancelInput,
) => Promise<CancelBroadcastResponseSuccess>;

export type GetBroadcastsIdRecipientsInput = {
  db: Db;
  env: Env;
  user: AuthUser | null;
  params: { id: string };
...
export type GetBroadcastsIdRecipientsHandler = (
  input: GetBroadcastsIdRecipientsInput,
) => Promise<ListBroadcastRecipientsResponseSuccess>;

export type GetBroadcastsIdClickedLinksInput = {
  db: Db;
  env: Env;
  user: AuthUser | null;
  params: { id: string };
...
export type GetBroadcastsIdClickedLinksHandler = (
  input: GetBroadcastsIdClickedLinksInput,
) => Promise<ListBroadcastClickedLinksResponseSuccess>;
...
export type PatchSegmentsIdInput = {
  db: Db;
  env: Env;
  user: AuthUser | null;
  params: { id: string };
  body: UpdateSegmentOptions;
};

export type PatchSegmentsIdHandler = (
  input: PatchSegmentsIdInput,
) => Promise<UpdateSegmentResponseSuccess>;
